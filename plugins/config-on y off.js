...
ღ *_𝐄𝐒𝐓𝐀𝐃𝐎_* *|* ${isEnable ? 'ACTIVADO' : 'DESACTIVADO'} 
ღ *_𝐏𝐀𝐑𝐀_* *|* ${isAll ? 'BOT' : isUser ? '' : 'CHAT'}`, wm, null, [[`${isEnable ? '🔕 DESACTIVAR' : '🔔 ACTIVAR'}`, `${isEnable ? `.off ${type}` : `.on ${type}`}`], ['𝙑𝙤𝙡𝙫𝙚𝙧 𝙖𝙡 𝙈𝙚𝙣𝙪́ | 𝘽𝙖𝙘𝙠 𝙩𝙤 𝙈𝙚𝙣𝙪 🍀', '.menu']],m)}
